# Hi, I'm Pankaj Damale 👋

### Backend-Focused Full Stack Developer

Building scalable APIs, SaaS systems, and AI-powered applications

---

## 🚀 About Me

I focus on building backend-heavy applications with clean architecture and real-world scalability.

* 🔧 Designing robust REST APIs
* 🧠 Working with databases & system design
* 🔐 Implementing authentication & RBAC systems
* 🤖 Integrating AI into applications
* ⚡ Building full-stack SaaS platforms

---

## 🛠 Tech Stack

**Languages**
![Languages](https://skillicons.dev/icons?i=java,python,js,ts)

**Backend**
![Backend](https://skillicons.dev/icons?i=nodejs,express,fastapi)

**Frontend**
![Frontend](https://skillicons.dev/icons?i=react,tailwind)

**Databases**
![Databases](https://skillicons.dev/icons?i=postgres,mongodb,mysql)

**Tools**
![Tools](https://skillicons.dev/icons?i=git,github,vscode,postman)

## 📚 Currently Learning

* Advanced FastAPI (async, background tasks, scaling)
* System Design for backend engineers
* Distributed systems & caching (Redis)
* SaaS architecture & billing systems

---

## 📫 Connect With Me

* GitHub: https://github.com/Pankaj-099
* LinkedIn: (add your link)
* Email: (optional)

---

⭐ Focused on backend engineering, scalability, and building production-ready systems.
